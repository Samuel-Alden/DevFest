...
function SegmentedToggle({ label, value, onChange, options }) {
  return (
    <div>
      ...
      <div className={`flex gap-1 bg-paper-dim rounded-lg p-1 ${label ? 'mt-1' : ''}`}>
        {options.map((opt) => (
          <button
            key={opt.value}
            type="button"
            ...
            className={`flex-1 text-sm font-medium py-1.5 rounded-md transition-colors ${
              value === opt.value ? 'bg-paper text-ink shadow-sm' : 'text-ink-soft hover:text-ink'
            }`}
          >
            {opt.label}
          </button>
...
function NotesSection({ form, setField, t }) {
  return (
    <Card title={t('notes')}>
      <TextAreaField value={form.notes} onChange={setField('notes')} placeholder={t('notes_placeholder')} rows={3} label="" />
    </Card>
  )
}
...
function PrimarySurveySection({ form, setField, t }) {
  ...
  return (
    <Card title={t('section_primary_survey')}>
      <SegmentedToggle label={t('airway_label')} value={form.airwayStatus} onChange={setField('airwayStatus')} options={statusOptions} />
      <SegmentedToggle
      ...
      <label className="flex items-center gap-3 rounded-lg border border-line px-3 py-2 cursor-pointer hover:bg-paper-dim">
        <input
          ...
          className="h-4 w-4 accent-brand"
        />
        <span className="flex-1 text-sm text-ink">{t('bleeding_trauma_label')}</span>
      </label>
      {form.bleedingTrauma && (
        <TextAreaField
          label=""
...
function MedicalHistorySection({ form, setField, t }) {
  return (
    <Card title={t('section_medical_history')}>
      ...
      <TextAreaField label={t('drug_allergies')} value={form.drugAllergies} onChange={setField('drugAllergies')} />
      <TextAreaField label={t('comorbidities')} value={form.comorbidities} onChange={setField('comorbidities')} />
      <TextAreaField label={t('current_medications')} value={form.currentMedications} onChange={setField('currentMedications')} />
    </Card>
  )
}
...
export function IntakePage({ onSubmitted }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const severity = assessmentEscalatesSeverity(assessment) ? 'red' : computeSeverity(form.symptoms)
        ...
        <div className="max-w-3xl mx-auto p-4 pb-24">
          {lastResult && (
            ...
            >
              ...
              <p className="text-sm text-ink-soft mt-1">{lastResult.synced ? t('sent_immediately') : t('saved_offline')}</p>
